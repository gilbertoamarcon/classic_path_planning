% function [num_expanded_nodes, path_size, path] = a_star_4d(map, start, goal, epsilon)
% A* 4D with tuned Euclidian distance as admissible heuristic
...
function [num_expanded_nodes, path_size, path] = a_star_4d(map, start, goal, epsilon)
    ...
    for i=1:IT_LIM
        ...
        % Expanding neighbors
        nbrs = get_neighbors_dynamic(map, node);
    ...
    for i=1:IT_LIM
        ...
        [x,y,dx,dy] = dynamic_state_from_index(map,node);
